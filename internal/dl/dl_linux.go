...
// The libdl imports live in dl_linux_glibc.go (default) or
// dl_musl_<arch>.go (-tags goffi_musl), so each libc flavor can name its own
// SONAMEs while these constants stay shared.
