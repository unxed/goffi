...
//go:build linux && !android && !goffi_static && !goffi_universal && goffi_musl && arm64

// Dynamic symbol imports for Linux/musl (Alpine and friends).
//
// musl ships the entire POSIX surface -- dlopen, pthreads, libm, errno --
// in one object whose name embeds the architecture: libc.musl-aarch64.so.1.
// There is no libdl.so.2 and no libc.so.6 on a musl system, so the glibc
// directives in dl_linux_glibc.go make the process fail to start ("Error
...
//go:cgo_import_dynamic goffi_dlopen dlopen "libc.musl-aarch64.so.1"
//go:cgo_import_dynamic goffi_dlsym dlsym "libc.musl-aarch64.so.1"
//go:cgo_import_dynamic goffi_dlerror dlerror "libc.musl-aarch64.so.1"
//go:cgo_import_dynamic goffi_dlclose dlclose "libc.musl-aarch64.so.1"

// Force dependency on musl libc
//go:cgo_import_dynamic _ _ "libc.musl-aarch64.so.1"

//go:cgo_dynamic_linker "/lib/ld-musl-aarch64.so.1"
