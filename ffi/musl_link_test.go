...
// goToolPath returns the go command from the active GOROOT (as reported by
// "go env GOROOT"), falling back to whatever "go" resolves to on PATH.
// runtime.GOROOT is deprecated since Go 1.24, so the value is queried from
// the tool at run time instead.
